...
const char * const LITELIRA_CONF_FILENAME = "litelira.conf";
const char * const LITELIRA_PID_FILENAME = "litelirad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "litelirad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LiteLira
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LiteLira
    // Mac: ~/Library/Application Support/LiteLira
    // Unix: ~/.litelira
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LiteLira";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LiteLira";
#else
    // Unix
    return pathRet / ".litelira";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LITELIRA_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", LITELIRA_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure LiteLira Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("LiteLira Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The LiteLira Core developers";
    }
