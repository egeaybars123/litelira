#!/usr/bin/env bash
# Copyright (c) 2016 The LiteLira Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   liteliracore.org and litelira.org and compares them.
...
WORKINGDIR="/tmp/litelira_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://liteliracore.org"
HOST2="https://litelira.org"
BASEDIR="/bin/"
VERSIONPREFIX="litelira-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'litelira-',
...
if [ $? -ne 0 ]; then
   echo "litelira.org failed to provide signature file, but liteliracore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "litelira.org and liteliracore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LiteLira Core binary release signing key installed?"
   fi
